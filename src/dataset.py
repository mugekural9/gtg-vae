...
def get_target2_fields(fields=None):
  if fields is None:
    fields = {}
...
def load_fields_from_vocab(vocab):
  ...
  # with open("vocab_src_dict.txt", 'w') as f:     
  # #       for token, index in fields["src"].vocab.stoi.items():
  #   f.write(json.dumps(fields["src"].vocab.stoi))
...
def load_dataset(corpus_type, task_type, opt):
  ...
  def __init__(self, fields, src_examples_iter, tgt_examples_iter,
    ...
    def filter_pred(example):
      ...
      if hasattr(example, "tgt"):
        ...
        return 0 < len(example.src) <= src_seq_length \
        and 0 < len(example.tgt2) <= tgt_seq_length
