...
def build_optim(model, opt, checkpoint):
    ...
    if opt.train_from and opt.reset_optim != 'all':
        ...
        if opt.reset_optim != 'states':
            ...
            if opt.reset_optim == 'keep_states':
                ...
                optim.original_lr2 = opt.learning_rate2
                ...
                optim.warmup_steps2 = opt.warmup_steps2
                optim.model_size = opt.dec_rnn_size
    else:
        optim = Optimizer(
            opt.optim, opt.learning_rate, opt.learning_rate2, opt.max_grad_norm,
            ...
            warmup_steps2=opt.warmup_steps2,
    ...
    #     optim.learning_rate = opt.learning_rate
    #     optim.original_lr = opt.learning_rate
    #     optim.original_lr2 = opt.learning_rate
    #     optim.max_grad_norm = opt.max_grad_norm
    #     optim.lr_decay = opt.learning_rate_decay
    #     optim.start_decay_steps = opt.start_decay_steps
    #     optim.decay_steps = opt.decay_steps
...
class Optimizer(object):
    ...
    def __init__(self, method, learning_rate, learning_rate2, max_grad_norm,
    ...
    def step(self, task_type):
        ...
        if self.decay_method == "noam":
            self._set_rate(
                ...
                (self.model_size ** (-0.5) *
                 min(_step ** (-0.5),
                     _step * warmup_steps**(-1.5))))
        # Decay based on start_decay_steps every decay_steps
        else:
            if ((self.start_decay_steps is not None) and (
                     _step >= self.start_decay_steps)):
                self.start_decay = True
            if self.start_decay:
                if ((_step - self.start_decay_steps)
                   % self.decay_steps == 0):
